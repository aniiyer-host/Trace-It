...
function StatBlock({
  ...
  return (
    <div className="space-y-2">
      <div
        className="font-bold tabular-nums tracking-tighter text-foreground"
...
export default function DonorDashboard() {
  ...
  const summary = useMemo(() => {
    ...
    const confirmedDonations = donations.filter(
      (d) => d.status === "delivered" || d.status === "disbursed",
    ).length;
    const successRate =
      donations.length > 0 ? (confirmedDonations / donations.length) * 100 : 0;
  ...
  //Below function removed unused donationId parameter
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[50vh] text-center space-y-6">
        ...
        <p className="text-muted-foreground text-lg max-w-md text-balance">
          Please sign in to view your donor portfolio.
        </p>
      ...
      {isZeroState ? (
        <div className="flex flex-col items-center text-center space-y-8 py-32 border-t border-foreground/10">
          ...
          <Button
            asChild
            size="lg"
            className="rounded-full px-8 py-6 text-lg h-auto mt-4"
          >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 border-t border-foreground/10 pt-16">
          ...
          <div className="lg:col-span-4 space-y-6">
            <h3 className="text-sm font-semibold uppercase tracking-widest text-foreground/50 mb-8">
              Active Deployments
            </h3>
            ...
            <div className="relative -mx-6 px-6 lg:mx-0 lg:px-0">
              <div className="flex lg:flex-col gap-4 overflow-x-auto snap-x snap-mandatory lg:overflow-visible lg:snap-none pb-8 lg:pb-0 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                {fundedCampaigns.map((camp) => {
                  ...
                  const hasDelivered = campDonations.some(
                    (d) => d.status === "delivered",
                  );
                  const hasDisbursed = campDonations.some(
                    (d) => d.status === "disbursed",
                  );
                  const statusColor = hasDelivered
                    ? "bg-primary"
                    : hasDisbursed
                      ? "bg-blue-500"
                      : "bg-yellow-500";
                ...
                >
                  <div>
                    ...
                    <p className="text-lg text-foreground/50">
                      Trace trajectory for this allocation
                    </p>
                  </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-12 lg:gap-0">
        {journey.map((step, idx) => {
          ...
          return (
            <div
              key={step.id}
              className="relative flex lg:flex-col items-center lg:items-start gap-6 lg:gap-4"
            >
              {/* Connecting Line */}
              {!isLast && (
                <>
                    ...
                    animate={
                      isCompleted
                        ? {
                            scale: [1, 1.15, 1],
                            boxShadow: [
                              "0px 0px 0px rgba(10,26,47,0)",
                              "0px 0px 15px rgba(10,26,47,0.3)",
                              "0px 0px 0px rgba(10,26,47,0)",
                            ],
                          }
                        : {}
              ...
              <div
                className={cn(
                  ...
                  step.isAttestation && isCompleted
