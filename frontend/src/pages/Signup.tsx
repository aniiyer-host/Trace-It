...
export default function Signup() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    if (!email.trim() || !password.trim() || !fullName.trim()) {
      toast({
        title: "Email, password, and full name are required",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      // Pan KYC
      if (pan.trim()) {
        await apiService.auth.submitKyc(pan.trim());
      }
    ...
    <div className="w-screen min-h-[calc(100dvh-3.5rem)] relative left-1/2 -ml-[50vw] -mt-8 flex flex-col md:flex-row bg-background">
      ...
      <div className="hidden md:flex md:w-1/2 bg-foreground text-background flex-col justify-end p-12 lg:p-24">
        ...
        <p className="text-xl lg:text-2xl font-medium text-background/80 max-w-md text-balance">
          Join the network for transparent, cryptographically verifiable
          charitable giving.
        </p>
      </div>
          ...
          {step === "success" ? (
            ...
            <>
              <motion.div variants={itemVariants} className="mb-12">
                <h2 className="text-4xl lg:text-5xl font-bold tracking-tight text-foreground mb-3">
                  Apply
                </h2>
                <p className="text-lg text-muted-foreground font-medium">
              ...
              <div className="space-y-8">
                ...
                <form
                  onSubmit={
                    step === "details" ? handleContinueToKYC : handleSignup
                  }
                  className="space-y-8"
                >
                  <div
                    className={`space-y-8 transition-opacity duration-500 ${step === "kyc" ? "opacity-30 pointer-events-none" : ""}`}
                  >
                    ...
                    >
                      <div className="mb-6 space-y-2">
                        ...
                        <p className="text-sm text-muted-foreground font-medium leading-relaxed">
                          For donations over ₹10,000, we verify your identity.
                          Your PAN is hashed and never stored on-chain.
                        </p>
                      </div>
              ...
              <motion.div variants={itemVariants} className="mt-8">
                <p className="text-muted-foreground font-medium">
                  ...
                  <Link
                    to="/login"
                    className="text-foreground border-b border-foreground hover:text-muted-foreground hover:border-muted-foreground transition-colors pb-0.5"
                  >
                    Sign In
                  </Link>
                </p>
