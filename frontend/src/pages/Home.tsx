...
const CHAIN_STEPS = [
  {
    icon: HeartHandshake,
    title: "1. Donation Escrow",
    desc: "Funds are securely locked on-chain, pending milestone validation.",
  },
  {
    icon: Shield,
    title: "2. Milestone Triggers",
    desc: "NGOs unlock capital only by providing cryptographic proof of work.",
  },
  {
    icon: FileText,
    title: "3. Immutable Receipts",
    desc: "Cohort data and delivery anchoring are permanently recorded.",
  },
  {
    icon: CheckCircle2,
    title: "4. Final Attestation",
    desc: "Irrevocable sign-off that aid reached the intended beneficiary.",
  },
  {
    icon: Zap,
    title: "5. Zero-Fee Settlement",
    desc: "Instant value transfer bypassing traditional banking bleed.",
  },
];
...
export default function Home() {
  ...
  useGSAP(
    () => {
      ...
      tl.to(
        trackRef.current,
        { x: "-100vw", duration: transDur, ease: "power2.inOut" },
        0,
      );
      tl.from(
        panel2Ref.current,
      ...
      tl.to(
        trackRef.current,
        { x: "-200vw", duration: transDur, ease: "power2.inOut" },
        "<",
      );
      tl.from(
        panel3Ref.current,
      ...
      tl.to(
        trackRef.current,
        { x: "-300vw", duration: transDur, ease: "power2.inOut" },
        "<",
      );
      tl.from(
        panel4Ref.current,
      ...
      tl.to(
        trackRef.current,
        { x: "-400vw", duration: transDur, ease: "power2.inOut" },
        "<",
      );
      tl.from(
        panel5Ref.current,
  ...
  const impactMetrics = {
    ...
    totalDonors: campaigns.reduce(
      (sum, c) => sum + (Number(c.successDonationCount) || 0),
      0,
    ),
    activeCampaigns: campaigns.length,
    milestonesCompleted: campaigns.reduce(
      (sum, c) =>
        sum +
        (c.milestones || []).filter((m) => m.status === "delivered").length,
      0,
    ),
  };

  const handleDonate = (c: Campaign) => {
    ...
    <div className="font-sans selection:bg-foreground selection:text-background">
      ...
      >
        ...
        >
          ...
          <div
            ref={panel1Ref}
            className="w-[100vw] h-full flex-shrink-0 flex flex-col items-center justify-center relative p-6"
          >
            ...
            <p className="text-2xl md:text-4xl text-foreground/60 max-w-[40ch] leading-relaxed text-pretty text-center font-medium mt-12">
              We anchor every donation milestone on-chain. Verify exactly where
              your money went, down to the last rupee.
            </p>
          ...
          <div
            ref={panel2Ref}
            className="w-[100vw] h-full flex-shrink-0 flex items-center justify-center relative"
          >
            <div
          ...
          <div
            ref={panel3Ref}
            className="w-[100vw] h-full flex-shrink-0 flex items-center justify-center relative p-6"
          >
            <div className="grid grid-cols-2 gap-y-24 gap-x-32 text-center max-w-5xl">
              <StatBlock
                value={impactMetrics.totalRaised}
                label="Rupees Raised"
                prefix="₹"
                trigger={triggerStats}
              />
              <StatBlock
                value={impactMetrics.activeCampaigns}
                label="Active Campaigns"
                trigger={triggerStats}
              />
              <StatBlock
                value={impactMetrics.milestonesCompleted}
                label="Milestones Completed"
                trigger={triggerStats}
              />
              <StatBlock
                value={impactMetrics.totalDonors}
          ...
          >
            <div className="w-full max-w-[1400px] mx-auto space-y-16">
              <div className="text-center space-y-6">
                <h2 className="text-6xl md:text-8xl font-bold tracking-tighter text-foreground text-balance">
                  Fund Verified Impact
                </h2>
                <p className="text-2xl text-foreground/50 text-pretty">
                  Select a campaign to begin tracking your contribution
                  on-chain.
                </p>
              </div>
              <div className="flex justify-center gap-8 w-full">
                {campaignsLoading
                  ? [1, 2, 3].map((i) => (
                      <div
                        key={i}
                        className="w-[400px] h-[500px] shrink-0 bg-foreground/5 rounded-[2.5rem] animate-pulse"
                      />
                    ))
