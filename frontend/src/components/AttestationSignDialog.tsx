  ...
  const handleSignAttestation = async () => {
    ...
    try {
      ...
      toast({
        ...
        //For this to work above added {id?: string } | undefined to the result type
      ...
      //changed any to unknown to avoid eslint error
      ...
      if (typeof error === "object" && error !== null && "response" in error) {
        ...
        if (
          response?.status === 422 &&
          response.data?.error === "BENEFICIARY_MISMATCH"
        ) {
          setBeneficiaryError(
            response.data.message ?? "Beneficiary ID does not match.",
          );
          setBeneficiaryId("");
          // Do NOT close dialog
          // Do NOT show generic error toast
        ...
        <div className="p-8 pb-6 border-b border-foreground/5 bg-foreground/[0.02]">
          <DialogHeader>
            ...
            <DialogDescription className="text-foreground/60">
              You are about to sign a permanent, publicly verifiable record on
        ...
        <div className="p-8 space-y-8">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            >
              ...
              <span className="text-xs opacity-80 leading-tight">
                Confirm platform funds reached your NGO bank account.
              </span>
            </button>
            ...
            >
              ...
              <span className="text-xs opacity-80 leading-tight">
                Confirm funds were successfully delivered to beneficiary.
              </span>
            </button>
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="bg-foreground/5 p-4 rounded-xl border border-foreground/10 font-mono text-sm space-y-2 overflow-x-auto">
              <div className="flex">
                <span className="text-foreground/40 w-24">target:</span>
                <span className="text-foreground font-semibold">
                  {donation.id}
                </span>
              </div>
              <div className="flex">
                <span className="text-foreground/40 w-24">campaign:</span>
                <span className="text-foreground">
                  {donation.campaignTitle}
                </span>
              </div>
              <div className="flex">
                <span className="text-foreground/40 w-24">amount:</span>
                <span className="text-foreground">
                  ₹{donation.amount.toLocaleString()}
                </span>
              </div>
              <div className="flex">
                <span className="text-foreground/40 w-24">claim_type:</span>
                <span
                  className={cn(
                    "font-bold",
                    attestationType === "receipt"
              ...
              <div className="flex">
                <span className="text-foreground/40 w-24">signer:</span>
                <span className="text-foreground">
                  {ngoName} (Keypair active)
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-start gap-3 bg-foreground/5 p-4 rounded-xl text-sm text-foreground/80">
            ...
            <p>
              By signing this payload, your organization creates an undeniable
              cryptographic proof of{" "}
              {attestationType === "receipt" ? "receipt" : "delivery"}. This
            ...
            <div className="space-y-4 pt-4 border-t border-foreground/5">
              <div>
                ...
                <p className="text-xs text-foreground/60 mt-1">
                  Enter the wallet ID of the beneficiary you delivered funds to.
                  This must match the ID registered when the campaign was
                  created.
                </p>
              </div>
          ...
          <div className="flex flex-col-reverse sm:flex-row justify-end gap-3 pt-4">
            ...
            >
              {loading ? (
                ...
                <>
                  <Shield className="mr-2 h-4 w-4" /> Sign & Broadcast
                  Attestation
                </>
              )}
            </Button>
