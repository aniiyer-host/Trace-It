...
process.env.AES_BENEFICIARY_KEY = process.env.AES_BENEFICIARY_KEY || crypto.randomBytes(32).toString('hex');
...
describe('Beneficiary ID Feature', () => {
  ...
  beforeAll(async () => {
    ...
    const otherNgo = await prisma.profile.create({
      data: { email: 'other_ngo@test.com', role: UserRole.CHARITY, passwordHash: 'hash', ngoStatus: 'ACTIVE' }
    });
    otherNgoId = otherNgo.id;
    otherNgoToken = generateToken(otherNgoId);

    const admin = await prisma.profile.create({
      data: { email: 'admin_bene@test.com', role: UserRole.ADMIN, passwordHash: 'hash' }
    });
    adminId = admin.id;
    adminToken = generateToken(adminId);

    const donor = await prisma.profile.create({
      data: { email: 'donor_bene@test.com', role: UserRole.DONOR, passwordHash: 'hash' }
    });
    donorId = donor.id;
  ...
  afterAll(async () => {
    ...
    await prisma.profile.deleteMany({ where: { id: { in: [ngoId, otherNgoId, adminId, donorId] } } });
  ...
  describe('Campaign Response Stripping', () => {
    it('beneficiaryIdEncrypted is never present in the response body of POST /api/charity/campaigns', async () => {
      ...
      // clean up
      await prisma.campaign.delete({ where: { id: res.body.id } });
    });
