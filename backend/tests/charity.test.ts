...
describe('Charity API Integration Tests', () => {
  ...
  beforeAll(async () => {
    ...
    // Generate JWT
    charityToken = jwt.sign({ userId: user.id }, JWT_ACCESS_SECRET, { expiresIn: '1h' });
  });

  afterAll(async () => {
    // Clean up
    await prisma.document.deleteMany({ where: { ownerId: charityUserId } });
    await prisma.beneficiaryCohort.deleteMany({ where: { ngoId: charityUserId } });
  ...
  test('POST /api/charity/onboard - update NGO details', async () => {
    ...
    const pendingToken = jwt.sign({ userId: pendingUser.id }, JWT_ACCESS_SECRET);
