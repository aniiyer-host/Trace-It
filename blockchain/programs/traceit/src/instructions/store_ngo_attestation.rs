...
pub struct StoreNgoAttestation<'info> {
    #[account(
        init,
        payer = authority,
        space = 8 + AttestationAccount::INIT_SPACE,
        seeds = [b"attestation", donation_id.replace("-", "").as_bytes(), ngo_id.replace("-", "").as_bytes(), b"receipt"],
        bump,
    )]
...
pub fn handler(
    ...
    attestation.attestation_type = 0; // 0 = Receipt
    attestation.beneficiary_id_hash = String::new();
    ...
    msg!(
        "TraceIt: NGO receipt attestation stored for donation {}",
